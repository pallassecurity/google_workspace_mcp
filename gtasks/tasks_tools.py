...
async def list_tasks(
    ...
    max_results: int = Field(
        ...
        description="Maximum number of tasks to return. (default: 20, max: 10000).",
    ),
    page_token: Optional[str] = Field(
        default=None, description="Token for pagination."
    ),
    show_completed: bool = Field(
        default=True,
        description="Whether to include completed tasks (default: True). Note that show_hidden must also be true to show tasks completed in first party clients, such as the web UI and Google's mobile apps.",
    ),
    show_deleted: bool = Field(
        default=False, description="Whether to include deleted tasks (default: False)."
    ),
    show_hidden: bool = Field(
        default=False, description="Whether to include hidden tasks (default: False)."
    ),
    show_assigned: bool = Field(
        default=False, description="Whether to include assigned tasks (default: False)."
    ),
    completed_max: Optional[str] = Field(
        default=None,
        description="Upper bound for completion date (RFC 3339 timestamp).",
    ),
    completed_min: Optional[str] = Field(
        default=None,
        description="Lower bound for completion date (RFC 3339 timestamp).",
    ),
    due_max: Optional[str] = Field(
        default=None, description="Upper bound for due date (RFC 3339 timestamp)."
    ),
    due_min: Optional[str] = Field(
        default=None, description="Lower bound for due date (RFC 3339 timestamp)."
    ),
...
async def create_task(
    ...
    notes: Optional[str] = Field(
        default=None, description="Notes/description for the task."
    ),
    ...
    previous: Optional[str] = Field(
        default=None, description="Previous sibling task ID (for positioning)."
    ),
) -> str:
    """
...
async def update_task(
    ...
    notes: Optional[str] = Field(
        default=None, description="New notes/description for the task."
    ),
    status: Optional[str] = Field(
        default=None, description='New status ("needsAction" or "completed").'
    ),
    due: Optional[str] = Field(
        default=None, description="New due date in RFC 3339 format."
    ),
) -> str:
    """
...
async def move_task(
    ...
    task_list_id: str = Field(
        description="The ID of the current task list containing the task."
    ),
    task_id: str = Field(description="The ID of the task to move."),
    parent: Optional[str] = Field(
        default=None, description="New parent task ID (for making it a subtask)."
    ),
    previous: Optional[str] = Field(
        default=None, description="Previous sibling task ID (for positioning)."
    ),
    destination_task_list: Optional[str] = Field(
        default=None, description="Destination task list ID (for moving between lists)."
    ),
) -> str:
    """
