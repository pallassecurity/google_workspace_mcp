...
async def search_drive_files(
    ...
    query: str = Field(description="The search query string. Supports Google Drive search operators."),
    page_size: int = Field(default=10, description="The maximum number of files to return. Defaults to 10."),
    drive_id: Optional[str] = Field(default=None, description="ID of the shared drive to search. If None, behavior depends on `corpora` and `include_items_from_all_drives`."),
    include_items_from_all_drives: bool = Field(default=True, description="Whether shared drive items should be included in results. Defaults to True. This is effective when not specifying a `drive_id`."),
    corpora: Optional[str] = Field(default=None, description="Bodies of items to query (e.g., 'user', 'domain', 'drive', 'allDrives'). If 'drive_id' is specified and 'corpora' is None, it defaults to 'drive'. Otherwise, Drive API default behavior applies. Prefer 'user' or 'drive' over 'allDrives' for efficiency."),
) -> str:
    """
...
async def list_drive_items(
    service,
    user_google_email: str = Field(description="The user's Google email address. Required."),
    folder_id: str = Field(default='root', description="The ID of the Google Drive folder. Defaults to 'root'. For a shared drive, this can be the shared drive's ID to list its root, or a folder ID within that shared drive."),
    page_size: int = Field(default=100, description="The maximum number of items to return. Defaults to 100."),
    drive_id: Optional[str] = Field(default=None, description="ID of the shared drive. If provided, the listing is scoped to this drive."),
    include_items_from_all_drives: bool = Field(default=True, description="Whether items from all accessible shared drives should be included if `drive_id` is not set. Defaults to True."),
    corpora: Optional[str] = Field(default=None, description="Corpus to query ('user', 'drive', 'allDrives'). If `drive_id` is set and `corpora` is None, 'drive' is used. If None and no `drive_id`, API defaults apply."),
) -> str:
    """
...
async def create_drive_file(
    ...
    content: Optional[str] = Field(default=None, description="If provided, the content to write to the file."),  # Now explicitly Optional
    folder_id: str = Field(default='root', description="The ID of the parent folder. Defaults to 'root'. For shared drives, this must be a folder ID within the shared drive."),
    mime_type: str = Field(default='text/plain', description="The MIME type of the file. Defaults to 'text/plain'."),
    fileUrl: Optional[str] = Field(default=None, description="If provided, fetches the file content from this URL."),  # Now explicitly Optional
) -> str:
    """
...
async def get_drive_file_permissions(
    service,
    user_google_email: str = Field(description="The user's Google email address. Required."),
    file_id: str = Field(description="The ID of the file to check permissions for."),
) -> str:
    """
...
async def check_drive_file_public_access(
    service,
    user_google_email: str = Field(description="The user's Google email address. Required."),
    file_name: str = Field(description="The name of the file to check."),
) -> str:
    """
