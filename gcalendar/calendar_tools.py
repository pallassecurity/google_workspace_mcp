...
async def get_events(
    ...
    calendar_id: str = Field(
        default="primary",
        description="The ID of the calendar to query. Use 'primary' for the user's primary calendar. Defaults to 'primary'. Calendar IDs can be obtained using `list_calendars`.",
    ),
    ...
    time_min: Optional[str] = Field(
        default=None,
        description="The start of the time range (inclusive) in RFC3339 format (e.g., '2024-05-12T10:00:00Z' or '2024-05-12'). If omitted, defaults to the current time. Ignored if event_id is provided.",
    ),
    time_max: Optional[str] = Field(
        default=None,
        description="The end of the time range (exclusive) in RFC3339 format. If omitted, events starting from `time_min` onwards are considered (up to `max_results`). Ignored if event_id is provided.",
    ),
    max_results: int = Field(
        default=25,
        description="The maximum number of events to return. Defaults to 25. Ignored if event_id is provided.",
    ),
    ...
    detailed: bool = Field(
        default=False,
        description="Whether to return detailed event information including description, location, attendees, and attendee details (response status, organizer, optional flags). Defaults to False.",
    ),
...
async def create_event(
    ...
    start_time: str = Field(
        description='Start time (RFC3339, e.g., "2023-10-27T10:00:00-07:00" or "2023-10-27" for all-day).'
    ),
    end_time: str = Field(
        description='End time (RFC3339, e.g., "2023-10-27T11:00:00-07:00" or "2023-10-28" for all-day).'
    ),
    calendar_id: str = Field(
    ...
    timezone: Optional[str] = Field(
        default=None, description='Timezone (e.g., "America/New_York").'
    ),
    attachments: List[str] = Field(
        default=[],
        description="List of Google Drive file URLs or IDs to attach to the event.",
    ),
    add_google_meet: bool = Field(
        default=False,
        description="Whether to add a Google Meet video conference to the event. Defaults to False.",
    ),
    reminders: Optional[Union[str, List[Dict[str, Any]]]] = Field(
        default=None,
        description="""JSON string or list of reminder objects. Each should have 'method' ("popup" or "email") and 'minutes' (0-40320). Max 5 reminders. Example: '[{"method": "popup", "minutes": 15}]' or [{"method": "popup", "minutes": 15}]""",
    ),
    use_default_reminders: bool = Field(
        default=True,
        description="Whether to use calendar's default reminders. If False, uses custom reminders. Defaults to True.",
    ),
    transparency: Optional[str] = Field(
        default=None,
        description='Event transparency for busy/free status. "opaque" shows as Busy (default), "transparent" shows as Available/Free. Defaults to None (uses Google Calendar default).',
    ),
) -> str:
    """
    Creates a new event.
...
async def modify_event(
    ...
    start_time: Optional[str] = Field(
        default=None,
        description='New start time (RFC3339, e.g., "2023-10-27T10:00:00-07:00" or "2023-10-27" for all-day).',
    ),
    end_time: Optional[str] = Field(
        default=None,
        description='New end time (RFC3339, e.g., "2023-10-27T11:00:00-07:00" or "2023-10-28" for all-day).',
    ),
    description: Optional[str] = Field(
        default=None, description="New event description."
    ),
    location: Optional[str] = Field(default=None, description="New event location."),
    attendees: List[str] = Field(
        default=[], description="New attendee email addresses."
    ),
    timezone: Optional[str] = Field(
        default=None, description='New timezone (e.g., "America/New_York").'
    ),
    add_google_meet: Optional[bool] = Field(
        default=None,
        description="Whether to add or remove Google Meet video conference. If True, adds Google Meet; if False, removes it; if None, leaves unchanged.",
    ),
    reminders: Optional[Union[str, List[Dict[str, Any]]]] = Field(
        default=None,
        description="""JSON string or list of reminder objects to replace existing reminders. Each should have 'method' ("popup" or "email") and 'minutes' (0-40320). Max 5 reminders. Example: '[{"method": "popup", "minutes": 15}]' or [{"method": "popup", "minutes": 15}]""",
    ),
    use_default_reminders: Optional[bool] = Field(
        default=None,
        description="Whether to use calendar's default reminders. If specified, overrides current reminder settings.",
    ),
    transparency: Optional[str] = Field(
        default=None,
        description='Event transparency for busy/free status. "opaque" shows as Busy, "transparent" shows as Available/Free. If None, preserves existing transparency setting.',
    ),
) -> str:
    """
